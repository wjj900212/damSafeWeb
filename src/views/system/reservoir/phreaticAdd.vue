<template>
  ...
  <a-modal title="添加场景" :visible="visible" width="750px" class="sceneStyle" :confirmLoading="uploading" okText="保存"
    ...
    <a-form :form="form" v-bind="formItemLayout">
      ...
      <a-form-item label="起始刻度" v-bind="formItemLayout">
        <a-input v-decorator="['startScale', { rules: [{ required: true, message: '请输入起始刻度' }] }]" />
      </a-form-item>
      <a-form-item label="结束刻度" v-bind="formItemLayout">
        <a-input v-decorator="['endScale', { rules: [{ required: true, message: '请输入结束刻度' }] }]" />
      </a-form-item>
    ...
    handleUpload () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          formData.append('startScale', values.startScale)
          formData.append('endScale', values.endScale)
          // for (let [a, b] of formData.entries()) {
          //   console.log(a, b)
          // }
          this.$upload('web/hiddenScene/addScene', formData).then((r) => {
            if (r.data.code === 1) {
                r.data.data.startScale=values.startScale
                r.data.data.endScale=values.endScale
