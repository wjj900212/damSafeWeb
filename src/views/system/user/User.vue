<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        <a-button @click="gotoUserRole" v-hasPermission="'user:add'">用户角色管理</a-button>
        <a-button type="primary" ghost @click="add" v-hasPermission="'user:add'">添加用户</a-button>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="userProjCount" slot-scope="text, record">
          <div><a @click="gotoProjList(record.userId)">{{text}}</a></div>
        </template>
        <template slot="userFollowCount" slot-scope="text, record">
          <div><a @click="gotoProjList(record.userId)">{{text}}</a></div>
        </template>
        <template slot="operation" slot-scope="text, record">
          <a-icon v-hasPermission="'user:update'" type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="编辑"></a-icon>
          &nbsp;
          <!-- <a-icon v-hasPermission="'user:view'" type="eye" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="查看"></a-icon>
          &nbsp; -->
          <a-icon v-hasPermission="'user:delete'" type="delete" theme="twoTone" twoToneColor="#42b983" @click="userDelete(record)" title="删除"></a-icon>
...
export default {
  ...
  computed: {
    ...
    columns () {
      ...
      //   width: '10%',
      //   scopedSlots: { customRender: 'userProjCount' }
      //   // customRender: (text, row, index) => {
      //   //   return <a href="javascript:gotoProjList();">{{text}}</a>
      //   // },
      // }, {
      //   title: '关注项目数',
      //   width: '10%',
      //   dataIndex: 'userFollowCount',
      //   scopedSlots: { customRender: 'userFollowCount' }
      // },
      {
        title: '最近登录时间',
  ...
  methods: {
    gotoProjList (userId) {
      this.$router.push('/proj/list?userId=' + userId)
    },
    gotoUserRole () {
