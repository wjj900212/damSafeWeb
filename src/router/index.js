...
import LeftPageView from '@/views/common/LeftPageView'
...
router.beforeEach((to, from, next) => {
  ...
  if (token.length && user) {
    if (!asyncRouter) {
      if (!userRouter) {
        asyncRouter = require('./router.json')
        save('USER_ROUTER', asyncRouter)
        go(to, next)
        // request.get(`server/menu.php?action=menulist&username=${user.username}`).then((res) => {
